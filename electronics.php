...
$stmt->fetch(); //fetch all rows and returns the results set as an array 
$stmt->close();

$product_query = "SELECT * FROM products WHERE category = 'electronics'";
$stmt = $conn->prepare($product_query);
$stmt->execute();
$result = $stmt->get_result();

$products = [];
while ($row = $result->fetch_assoc()) {  //Returns an associative array of strings representing the fetched row. NULL if there are no more rows in result-set
    $products[] = $row;
}
$stmt->close();
$conn->close();
